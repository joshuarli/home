...
QEMU firmware is host-specific and is not downloaded or generated by this
repository. A local firmware pair under `dist/qemu/firmware/` is preferred when
present; otherwise, if it is not in one of the common Homebrew paths, provide
explicit paths:
