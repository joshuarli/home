...
- The target boots through UEFI and Alpine's packaged kernel hooks. It does
  not install GRUB. The root command line uses `root=PARTUUID=...`.
...
- The session is `tty1` autologin as `josh` -> guarded `home-session` -> dwl
  -> one foot terminal. Upstream dwl's `Alt+Shift+Return` is the
  second-terminal binding.
- The upstream stock v0.8 `Mod+p` example for `wmenu-run` remains dormant:
  `wmenu` is not a target dependency, and no local dwl config or wrapper is
  added merely to activate it.
...
- Alpine 3.24.1 has no `dwl` package. `Dockerfile` pins the Codeberg `dwl`
  v0.8 source checksum and builds its stock binary in a build-only stage;
  runtime libraries remain Alpine packages in `rootfs-packages.txt`. Do not
  add a repository-owned dwl package/config or follow a moving branch.
- The alternate-root build uses `apk --root --initdb --no-scripts` because the
  Buildx stage is the target architecture. `rootfs/configure.sh` and
  `build/build-rootfs.sh` must explicitly recreate required account, service,
  font and boot-hook side effects; a successful archive alone is not proof.
...
the ISO attached as CD media and the target as SATA/AHCI. It never uses
...
facts, screenshots, upstream Alt+Shift+Return behavior, terminal command output, recovery
